...
import Tape from '../assets/images/illu/seh-check-allgemein/entfernung.png';
...
export default function ReadyScreen(props: NativeStackScreenProps<RootStackParamList, 'Ready'>) {
  ...
  useEffect(() => {
    const { ttsKey } = VisionTests[visionTest!].readyScreen!;
  ...
  return (
    <Container style={[{ paddingBottom: headerHeight }, styles.container]}>
      ...
      <BigCard
        image={Tape}
        title={VisionTests[visionTest!].readyScreen!.title}
        description={VisionTests[visionTest!].readyScreen!.text}
      />
      <Footer
        countdownTime={AppConfig.countdownTimes.readyScreen}
        onCountdownFinished={() => {
